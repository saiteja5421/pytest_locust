pipeline {
    ...
    parameters {
        ...
        choice(
            name: 'CONFIG_FILE',
            choices: ['variables_sanity_scdev01','variables_sanity_scint','variables_sanity_prod','variables_reg1','variables_reg2','variables_reg1_scint','variables_reg2_scint'],        
            description: 'Select the variables files to cleanup'
    )
    }

    environment {
        CONTAINER_NAME = "atlas-service1-cleanup${env.BUILD_NUMBER}"
    ...
    stages {
        ...
        stage('Run service1 cleanup scripts') {
            steps {
                withCredentials([string(credentialsId: 'RP_API_KEY_2', variable: 'RP_API_KEY')]) {
                    script {
                        ...
                        if (params.ENABLE_REPORT_PORTAL) {
                            sh "${command} --reportportal /Medusa/utils/test_cleanup_scripts.py"
                        } else {
                            sh "${command} /Medusa/utils/test_cleanup_scripts.py"
                        }
                    }
                }
            }
