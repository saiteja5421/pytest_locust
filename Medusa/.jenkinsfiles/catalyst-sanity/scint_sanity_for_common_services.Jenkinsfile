pipeline {
    ...
    parameters {
        string(defaultValue: 'master',
                ...
                name: 'BRANCH_NAME',
                trim: true)
        string(defaultValue: 'refs/tags/s1-scint-may2024-v1',
                description: 'Default value is refs/tags/s1-scint-may2024-v1, type your tag name if you want to run build using a specific tag',
                name: 'TAG',
                trim: true)
        booleanParam(defaultValue: true,
                description: 'Send emails with the build status (true) or not (false)',
                name: 'SEND_EMAILS')
        string(defaultValue: 'service1-psg-qa@hpe.com, dscc_app_plat_product_owners@hpe.com, nithya.ganesan@hpe.com, siraj.shaik@hpe.com, cds-sc-sanity-regress-aaaaj6yar4dhjlumk4euvrznp4@hpe.org.slack.com',
    ...
    environment {
        CONTAINER_NAME = "atlas-sanity-scint-${env.BUILD_NUMBER}"
        CONFIG_FILE = 'variables_sanity_scint_common_services'
        ...
        stage('Run Sanity Tests') {
            steps {
                withCredentials([string(credentialsId: 'RP_API_KEY_2', variable: 'RP_API_KEY')]) {
                    script {
                        def command = "docker run -w /Medusa/tests/catalyst_gateway_e2e --name ${env.CONTAINER_NAME} --pull always -e RP_API_KEY=$RP_API_KEY -e JENKINS_URL=${env.JENKINS_URL} -e BUILD_URL=${env.BUILD_URL} -e SERVICE_VERSION=${env.SERVICE_VERSION} -e CONFIG_FILE=${env.CONFIG_FILE} -v $WORKSPACE/Medusa:/Medusa hub.docker.hpecorp.net/atlas-qa/e2e-catalyst-gw-mgr pytest -m common_service_tests -sv -c pytest_sanity_for_common_services_scint.ini --junitxml=logs/junit.xml"
                        if (params.ENABLE_REPORT_PORTAL) {
                            sh "${command} --reportportal /Medusa/tests/catalyst_gateway_e2e/test_sanity/test_sanity_for_common_services.py"
                        } else {
                            sh "${command} /Medusa/tests/catalyst_gateway_e2e/test_sanity/test_sanity_for_common_services.py"
                        }
                    }
                }
            }
