pipeline {
    ...
    parameters {
        string(defaultValue: 'master',
                ...
                name: 'BRANCH_NAME',
                trim: true)
        string(defaultValue: 'refs/tags/s1-scint-may2024-v3',
                description: 'Default value is tags/s1-scint-may2024-v3, type your tag name if you want to run build using a specific tag',
                name: 'TAG',
                trim: true)
    ...
    environment {
        ...
        CONFIG_FILE = 'variables_reg1_scint'
            ...
            steps {
                withCredentials([string(credentialsId: 'RP_API_KEY_2', variable: 'RP_API_KEY')]) {
                    script {
                        ...
                        def command = "docker run -w /Medusa/tests/catalyst_gateway_e2e --name ${env.CONTAINER_NAME}  --pull always -e RP_API_KEY=$RP_API_KEY -e JENKINS_URL=${env.JENKINS_URL} -e BUILD_URL=${env.BUILD_URL} -e SERVICE_VERSION=${env.SERVICE_VERSION} -e CONFIG_FILE=${env.CONFIG_FILE} -v $WORKSPACE/Medusa:/Medusa hub.docker.hpecorp.net/atlas-qa/e2e-catalyst-gw-mgr python -m pytest -sv -c pytest_scint_reg1.ini --junitxml=logs/junit.xml"
