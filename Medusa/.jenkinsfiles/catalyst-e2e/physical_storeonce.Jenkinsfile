pipeline {
    ...
    environment {
        CONTAINER_NAME = "atlas-storeonce-pipeline-${env.BUILD_NUMBER}-${Math.abs(new Random().nextInt(101))}"
        CONFIG_FILE = 'variables_physical_storeonce'
            ...
            steps {
                withCredentials([string(credentialsId: 'RP_API_KEY_2', variable: 'RP_API_KEY')]) {
                    script {
                        ...
                        def command = "docker run -w /Medusa/tests/catalyst_gateway_e2e --name ${env.CONTAINER_NAME}  --pull always -e RP_API_KEY=$RP_API_KEY -e JENKINS_URL=${env.JENKINS_URL} -e BUILD_URL=${env.BUILD_URL} -e SERVICE_VERSION=${env.SERVICE_VERSION} -e CONFIG_FILE=${env.CONFIG_FILE} -v $WORKSPACE/Medusa:/Medusa hub.docker.hpecorp.net/atlas-qa/e2e-catalyst-gw-mgr python -m pytest -sv -c pytest_storeonce_reg.ini --junitxml=logs/junit.xml"
                        if (params.ENABLE_REPORT_PORTAL) {
                            sh "${command} --reportportal /Medusa/tests/catalyst_gateway_e2e/test_scripts/storeonce_regression"
                        } else {
                            sh "${command} /Medusa/tests/catalyst_gateway_e2e/test_scripts/storeonce_regression"
                        }
                    }
                }
            }
