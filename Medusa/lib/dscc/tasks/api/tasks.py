...
class TaskManager:
    def __init__(self, user: User):
        ...
        # Updated to use the GLCP compliant tasks API: /data-services/v1beta1/async-operations/{id}
        self.url = self.dscc["url"] + "/data-services/" + self.dscc["beta-version"]
        self.path = self.common_services_api["tasks"]
    ...
    def get_tasks(self, date_time, time_offset_minutes=0, include_userId=True) -> TaskList:
        ...
        max_objects = 10000
        all_tasks: TaskList = TaskList(items=[], count=0, offset=0, total=0)
        for offset in range(0, max_objects, 100):
            # https://pages.github.hpe.com/cloud/storage-api/data-services/v1beta1/index.html#tag/async-operations/operation/ListAsyncOperations
            params = {
                "filter": filter,
                "offset": offset,
                "limit": 100,
            ...
            all_tasks.count += len(paged_tasks.items)
    ...
    def get_filtered_tasks(self, filter: str) -> TaskList:
        """Returns tasks filtered by provided filter value

        Args:
            filter (str): Expression to filter results

        Returns:
            TaskList: List of tasks
        """
        query_params: str = f"filter={filter}"
        path: str = f"{self.path}?{query_params}"
        response = get(self.url, path, headers=self.user.authentication_header)
        tasks_list = TaskList.from_json(response.text)
        return tasks_list

    def get_task_by_filter(self, filter=None):
        return get(
            self.url,
            self.path + filter,
    ...
    def get_task_source_resource_uuid(self, task_id: str, source_resource_type: str):
        ...
        assert task_object.source_resource is not None or task_object.source_resource_uri is not None
        # Tolerate resource type in either old (e.g. "csp-account") or new (e.g. "hybrid-cloud/csp-account") styles.
        assert (
            source_resource_type in task_object.source_resource.type
            if task_object.source_resource
            else source_resource_type in task_object.source_resource_uri
        )
        source_resource = (
            task_object.source_resource if task_object.source_resource else task_object.source_resource_uri
        )
        index = source_resource.rfind("/")
        assert index != -1
        prefix, resource_uuid = (
            source_resource[: index + 1],
            source_resource[index + 1 :],  # noqa: E203
        )
    ...
    def get_tasks_by_name_and_resource_uri_with_no_offset(
        ...
        params = f"offset=0&limit=10&sort=createdAt desc&filter=createdAt gt {datetime_formated} and sourceResourceUri eq '{resource_uri}'"
        response = get(self.url, self.path, params, headers=self.user.authentication_header)
        logger.info(f"filter task response {response.text}")

        return_list = TaskList(items=[], count=0, offset=0, total=0)
    ...
    def get_tasks_by_name_and_resource(
        ...
        for offset in range(0, max_objects, 100):
            ...
            for task in task_list.items:
                logger.info(f"Task Name = {task.name}: {task.display_name}")
                if not task.source_resource and not task.source_resource_uri:
                    logger.info("No task.source_resource.resource_uri field")
                    continue
                if task_name in task.display_name and (
                    (task.source_resource and resource_uri in task.source_resource.resource_uri)
                    or resource_uri in task.source_resource_uri
                ):
                    logger.info(f"Adding item: {task}")
                    return_list.items.append(task)
                    return_list.total += 1
