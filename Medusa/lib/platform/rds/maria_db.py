...
class MariaDB(AbstractDatabase):
    ...
    def initialize_db_properties(self, database, user, password, host, port):
        ...
        if not self.connection:
            try:
                # NOTE: Connecting to MariaDB with PyMySQL library, just need to specify "mariadb+pymysql:" in set_db_connection()
                # mariadb library does not work through python automation currently (only manually)
                # can refer to https://linuxhint.com/connect_mariadb_pymysql/
                self.connection = pymysql.connect(
                    host=host, user=user, password=password, port=port, connect_timeout=300
                )
    ...
    def set_db_connection(self, db_name):
        ...
        if (not self.dbConnection) or (self.db_name and db_name != self.db_name):
            connstr = f"mariadb+pymysql://{self.user}:{self.password}@{self.host}:{self.port}/{db_name}"
    ...
    def get_size_of_db_in_MB(self, db_name=None):
        ...
        query = "SELECT table_schema 'Database Name', ROUND(SUM(data_length + index_length) / 1024 / 1024, 2) 'Size in MiB' FROM information_schema.tables GROUP BY table_schema;"
        rows = self.execute_sql_query(query)
    ...
    def insert_blob(self, db_name, file_path, table_names: list, num_of_records=1):
        ...
        Args:
        db_name: Name of DB
        table_names: List of names of tables in which blob is to be inserted
        num_of_records: count of records to be inserted
        """
        self.set_db_connection(db_name)
        self.dbConnection.execute("SET max_statement_time=0")
        ...
        for table in table_names:
            df.to_sql(table, con=self.dbConnection, index=False, if_exists="append", dtype=dtype)
            logger.info(f"Inserted {num_of_records} records in table {table}")
    ...
    def delete_table(self, db_name, table_names):
        ...
        table_names: list of table names to be deleted
        """
        for table in table_names:
            logger.info(f"Dropping table {table}")
            query = f"DROP table IF EXISTS {table};"
            super().execute_sql_query_no_return(query=query)
