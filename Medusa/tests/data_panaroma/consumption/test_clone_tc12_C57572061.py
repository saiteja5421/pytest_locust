...
from lib.dscc.data_panorama.consumption.models.clones import ClonesUsageTrend, TotalClonesUsage
...
def test_volume_tc12_C57572061(context: context, granularity):
    ...
    logger.info("PQA Testing -> Consumption -> volumes -> TC12_C57572061")
    logger.info("TC12, Jira ID: C57572061")
    ...
    # Get Array data
    array_clone_usage_trend = acp_obj.get_clones_usage_trend(
        start_date=start_date,
        end_date=end_date,
        granularity=granularity,
    )
    logger.info(array_clone_usage_trend.total)
    write_to_json(
    ...
    api_clone_usage_trend = wastage_array_obj.get_all_response_volume_path_parameter(
        func=clone_obj.get_clones_usage_trend,
        startTime=time_interval["starttime"],
        endTime=time_interval["endtime"],
        granularity=granularity,
    )
    logger.info(api_clone_usage_trend.total)
    write_to_json(
...
def _verify_clone_usage_trend(array_clone_usage_trend, api_clone_usage_trend):
    ...
    ), f"Total Clone Usage count is not matching. API: {api_clone_usage_trend.total} and {array_clone_usage_trend.total}"
    params = list(TotalClonesUsage.__dataclass_fields__.keys())
    ...
    for act_vol in array_clone_usage_trend.items:
        timestampflag = False
        for exp_vol in api_clone_usage_trend.items:
