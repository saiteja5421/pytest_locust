"""
Test Case ID C57482014
Test Description: TC9:- To verify customer is able to view activity details of all Thin provision volumes based on the IO activity range.
"""
...
@mark.validated
@mark.order(1045)
def test_volume_tc9_C57482014(context: context):
    """
    Test Description: TC9:- To verify customer is able to view activity details of all Thin provision volumes based on the IO activity range.

    API validated:

    curl --location 'http://CCS_DEV_URL/data-observability/v1alpha1/volumes-activity-trend?offset=0&limit=1000&filter=provisionType%20eq%20Thin' --header 'Authorization: Bearer TOKEN

    """
    logger.info("Start of test case")
    logger.info("PQA Testing -> Consumption -> Volumes -> TC9_C57482014")
    """
    Define the volume provision type of which volume activity trend to be fetched.
    Test case provides details of all those Thin provision volumes where IOActivity is within the defined range.
    """
    ...
    """
    temp, minmax_calculate = arr_spark.get_volumes_activity_trend_by_io(provisionType=tp_provisiontype)
    ...
    """
    tc_vol_activity_trend_expected, temp_activity_trend = arr_spark.get_volumes_activity_trend_by_io(
    ...
    write_to_json(
        ...
        path=f"{api_obj.json_path}/arr_{Path(__file__).stem}.json",
        sort_by="id",
    )
    """
    ...
    api_vol_activity_trend = api_obj.get_all_response_volactivity(
    ...
    write_to_json(
        ...
        path=f"{api_obj.json_path}/api_{Path(__file__).stem}.json",
        sort_by="id",
    )
