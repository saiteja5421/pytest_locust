...
@mark.validated
@mark.order(2035)
def test_clone_tc7_C57483231(context: Context):
    """
    Test Description:
        To verify customer is able to view clone activity details of all Thick provisioned type based on the IO range
    Automation blocks:
    ...
    provision_type   : str   - clone provisioning type(Thick)
    ...
    logger.info("PQA Testing -> Consumption -> clones -> TC7_C57483231")

    provision_type: str = "Thick"
    min_max_list = []
    ...
    # Get Array data
    array_clone_activity_trend, array_clone_activity_trend_dict = array_obj.get_clones_activity_trend_by_io(
    ...
    print(
        f"array_clone_activity_trend: {array_clone_activity_trend} api_clone_activity_trend {api_clone_activity_trend} "
    )
    ...
    print(
        f"Total records found as per input (IO {min_io} - {max_io} and {provision_type}): Array: {array_clone_activity_trend.total} API: {api_clone_activity_trend.total}"
    )
                    ...
                    if param != "activityTrendInfo":
                        if param in ["ioActivity","utilizedPercentage"]:
                            ...
                            assert record.__getattribute__(param) == item.__getattribute__(
                                param
                            ), f"API {param} value is not matching array value. API: {item.__getattribute__(param)} and Expected: {record.__getattribute__(param)}"
                    else:
                        ...
                        for api_item in item.activityTrendInfo:
                            ...
                            for array_item in record.activityTrendInfo:
                                if api_item.timeStamp == array_item.timeStamp:
                                    ...
                                    ), f"API ioactivity of {item.name} not matching with array data. Activity trend info from API: {api_item} \n Expected data: {array_item}"
                                    activity_trend_match = True
                            assert (
                                activity_trend_match == True
                            ), f"API output not found in Expected data list. \n API item: {api_item} Expected data: {array_item}"
                match_flag = True
            if match_flag:
                break
