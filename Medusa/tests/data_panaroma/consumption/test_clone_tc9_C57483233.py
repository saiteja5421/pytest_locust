...
@mark.validated
@mark.order(2045)
def test_clone_tc9_C57483233(context: Context):
    """
    Test Description:
        To verify customer is able to view clone activity details of all Thick provisioned type based on the clone size
    Automation blocks:
    ...
    logger.info("PQA Testing -> Consumption -> clones -> TC9_C57483233")
    ...
    for record in minmax_calculate.iterrows():
        min_max_list.append(record[1]["cloneusedsize"])
    min_max_list.sort()
    min_clone_size = randint(0, min_max_list[int(len(min_max_list) / 2) - 1])
    max_clone_size = randint(min_max_list[int(len(min_max_list) / 2) - 1], min_max_list[-1])
