...
from tests.steps.aws_protection.policy_manager_steps import (
    protect_standard_assets_with_protection_policy,
    unprotect_standard_assets_from_protection_policies,
...
def context():
    ...
    # Unprotect assets (Delete Protection Job & Protection Policy)
    protection_policy = test_context.policy_manager.get_protection_policy_by_name(
        protection_policy_name=test_context.sanity_policy
    )
    if protection_policy:
        unprotect_standard_assets_from_protection_policies(test_context)
        delete_protection_policy(test_context, policy_id=protection_policy.id)
...
@mark.sanity
@mark.order(10)
def test_register_account(context: SanityContext):
    """
    Integration Test Steps:
        Register Customer Account -> Verify Customer Account -> Create Protection Policy -> Create Protection Job
        -> Delete Protection Job -> Delete Protection Policy -> Delete Customer Account
    """
    ...
    # Protect assets (Create Protection Job)
    protect_standard_assets_with_protection_policy(context, protection_policy_id=protection_policy_id)

    # Unprotect assets (Delete Protection Job & Protection Policy)
    unprotect_standard_assets_from_protection_policies(context)
    delete_protection_policy(context, policy_id=protection_policy_id)

    # Deleting created account
    delete_csp_account_with_expectation(context, csp_account_id=csp_account.id)

    # Validating that the deleted account is no longer present
    account = get_csp_account_by_csp_name(context, account_name=aws_account_name, is_found_assert=False)
