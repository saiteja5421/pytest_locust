"""
TestRail ID - C57581949: 
    Parallel Protection Store/Protection Store Gateway VM creation on same vcenter with different users from a single tenant
"""
...
def context():
    context_for_user_one = Context(test_provided_user=ProvidedUser.user_one)
    context_for_user_two = Context(test_provided_user=ProvidedUser.user_two)
    ...
    context_for_user_two.nic_data1["network_address"] = context_for_user_two.nic_data1["additional_network_address1"]
    yield context_for_user_one, context_for_user_two
    perform_cleanup(context_for_user_one)
    perform_cleanup(context_for_user_two)


def user_behaviour(context: Context):
    create_protection_store_gateway_vm(
        context,
    ...
    create_protection_template(context, cloud_region=AzureLocations.AZURE_centralindia)
...
@mark.order(500)
@mark.deploy
def test_parallel_deploy_on_same_vc_multi_user(context, vm_deploy, shutdown_all_psgw):
    """
    TestRail ID - C57581949:
    Parallel Protection Store/Protection Store Gateway VM creation on same vcenter with different users from a single tenant
    """
