...
def send_restore_requested_event(
    ...
    headers.update(
        {
            ...
            "ce_type": b"cvsa.v1.CVSARestoreBatchRequestedEvent",
            "ce_partitionkey": kafka_manager.account_id,
            "ce_customerid": kafka_manager.account_id,
        }
    )
    requested_event = cvsa_manager_pb2.CVSARestoreBatchRequestedEvent()
    ...
    logger.info(f"Send message: cvsa.v1.CVSARestoreBatchRequestedEvent, customer_id: {kafka_manager.account_id}")
...
def verify_restore_requested_event(
    ...
    event_type = "cvsa.v1.CVSARestoreBatchRequestedEvent"
    logger.info(
    ...
    logger.info(f"Message found cvsa.v1.CVSARestoreBatchRequestedEvent: {message}")
    ...
    logger.info("Message verified cvsa.v1.CVSARestoreBatchRequestedEvent")


def send_restore_requested_event_ms365(
    kafka_manager: KafkaManager,
    headers: dict = None,
    cloud_region: CloudRegions = AWS_REGION_1_ENUM,
    update_offsets=True,
    ...
    headers.update(
        {
            "ce_id": kafka_manager.account_id,
            "ce_type": b"cvsa.v1.CVSAMicrosoft365RestoreBatchRequestedEvent",
            "ce_partitionkey": kafka_manager.account_id,
            "ce_customerid": kafka_manager.account_id,
        }
    )
    requested_event = cvsa_manager_pb2.CVSAMicrosoft365RestoreBatchRequestedEvent()
    set_requested_event_base(kafka_manager, requested_event, cloud_region)
    logger.info(
        f"Send message: cvsa.v1.CVSAMicrosoft365RestoreBatchRequestedEvent, customer_id: {kafka_manager.account_id}"
    )
