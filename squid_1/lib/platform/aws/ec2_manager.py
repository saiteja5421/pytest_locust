...
from common.enums.cloud_provider import CloudProvider
from common.enums.ec2_type import Ec2Type
from lib.platform.aws.client_config import ClientConfig
from lib.platform.aws.models.address import Address
from lib.platform.aws.models.instance import Tag, Instance
from lib.platform.aws.models.security_group import SecurityGroup
...
class EC2Manager:
    ...
    def _get_disk_dataclass(self, ec2_volume: "ec2.Volume") -> CloudDisk:
        return CloudDisk(
            name=ec2_volume.volume_id,
            disk_size_bytes=gib_to_bytes(ec2_volume.size),
            tags=ec2_volume.tags,
            instance_id=ec2_volume.attachments[0]["InstanceId"],
            device=ec2_volume.attachments[0]["Device"],
            state=ec2_volume.attachments[0]["State"],
        )
    ...
    def get_disk(self, disk_id: str) -> CloudDisk:
        ec2_volume = self.ec2_resource.Volume(disk_id)
        return self._get_disk_dataclass(ec2_volume=ec2_volume)

    def create_instance(
        self, image_id: str, tags: list[Tag], subnet_tag: Tag = "", instance_type: str = "t2.micro"
    ) -> CloudInstance:
    ...
    def get_ebs_volumes_attached_to_instance(self, instance_id) -> list:
        instance = self.ec2_resource.Instance(instance_id)
        return [volume for volume in instance.volumes.all()]

    def get_ebs_volumes_id_attached_to_instance(self, instance_id) -> list:
