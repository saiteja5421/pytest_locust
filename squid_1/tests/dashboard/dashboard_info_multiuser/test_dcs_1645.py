...
from tests.dashboard.dashboard_info_multiuser.task import DashboardInfoTask
...
class LoadUser(HttpUser):
    ...
    def on_start(self):
        try:
            # api_client_cred, api_header = self.random_api_header()
            self.api_header = helpers.gen_token(credential_name="scdev-atlantia_psr")
            # self.api_client_cred = api_client_cred

        except Exception as e:
            logging.error(f"Exception is {e}")

    def random_api_header(self):
        config = helpers.read_config()
        api_users = config["testbed"]["users"]
        api_user = random.choice(api_users)
        api_client_cred = APIClientCredential(**api_user)
        api_header = ApiHeader(api_credential=api_client_cred, oauth2_server=config["testbed"]["cloud"]["oauth2server"])

        return api_client_cred, api_header
