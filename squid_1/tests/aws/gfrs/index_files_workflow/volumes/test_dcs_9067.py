"""
 Test case: DCS-9067
    FLR: [PSR] Run parallel index-files API on different native/cloud backups for same ebs volume. (5 operations)
    https://nimblejira.nimblestorage.com/browse/DCS-9067
Steps:
    1. Create multiple cloud and native backups of EBS Volume.
    2. Using locust framework for API stress testing call
        "<>/backup-recovery/v1beta1/csp-volumes/{id}/backups/{backupId}/index-files" in parallel different native backups of same ebs volume. (5 users)
...
from tests.aws.gfrs.index_files_workflow.volumes.task import (
    CSPVolumeBackupIndexFileRestoreTasks,
)
...
TAG_KEY: str = "Test_DCS_9067_" + str(uuid.uuid4())
TAG_VALUE: str = "Test_DCS_9067"
KEY_PAIR_NAME: str = "Test_DCS_9067_key_" + str(uuid.uuid4())
...
def on_test_start(environment, **kwargs):
    try:
        ...
        global rp_test_id
        global rp_logger
        test_case_name = "FLR Volume Workflow"
...
class LoadUser(HttpUser):
    ...
    tasks = [CSPVolumeBackupIndexFileRestoreTasks]
    ...
    def on_start(self):
        ...
        for aws_ec2_instance in aws_ec2_instance_list:
            ...
            ebs_map_list.append(asset_map)

        account.refresh_inventory()

        logger.info("\n----Step 4 Validate EC2 Instance and EBS Volume in DSCC -------")
        for asset_map in ebs_map_list:
            logger.info(f"Iterating over map {asset_map}")
            aws_ec2_instance_id = asset_map["aws_ec2_instance_id"]
            aws_ebs_volume_id = asset_map["aws_ebs_volume_id"]
            ...
            # Assign Protection Policy to EBS
            logger.info(
                f"\n----Step 5 Assign Protection Policy {self.protection_policy_name} to EBS {asset_map['aws_ebs_volume_id']}  -------"
            )
            protection_job_task_uri = protection_job.create_protection_job(
                asset_id=asset_map["csp_ebs_id"],
            ...
            protection_job_response = protection_job.get_protection_job_by_asset_id(asset_id=csp_ebs_id)
        ...
        for asset_map in ebs_map_list:
            ...
            csp_ebs_id = asset_map["csp_ebs_id"]
            backup_response_data = backups.get_all_csp_volume_backups(csp_ebs_id)
        ...
        self.current_map = ebs_map_list.pop()
        self.csp_volume_id = self.current_map["csp_ebs_id"]
    ...
    def on_stop(self):
        ...
        # Delete EBS Backups        # Delete EC2 Backups
        self.csp_ebs_id = self.current_map["csp_ebs_id"]
        ...
        if self.csp_ebs_id:
            ...
            if backup_response_data["total"] > 0:
                for backup in backup_response_data["items"]:
                    backup_id = backup["id"]
                    backups.delete_csp_volume_backup(self.csp_ebs_id, backup_id)
